...
last_edited_time: "2026-08-26T00:43:00.000Z"
...
- [2. 2번 문제](#2-2번-문제)
  - [1) 배수 찾기](#1-배수-찾기)
  - [2) 누적합 종료 조건](#2-누적합-종료-조건)
  - [3) 짝수만 제곱하여 출력](#3-짝수만-제곱하여-출력)
  - [4) 구구단](#4-구구단)
  - [5) 별 피라미드 출력](#5-별-피라미드-출력)
...
정수 `N(1 ≤ N ≤ 9)`이 주어졌을 때, <strong>`N` 단 구구단을 1부터 9까지 출력</strong>하는 프로그램을 작성하세요. 
...
영수증에 적힌 <strong>총 구매 금액</strong>이 실제 구매한 각 물건의 <strong>가격 × 개수 합계</strong>와 일치하는지 확인하는 프로그램을 작성하세요.
...
테스트 케이스(`T`)만큼 <strong>두 정수 `A`와 `B`를 입력받아, 각각의 합을 출력</strong>하는 프로그램을 작성하세요.

단, 출력 형식은 `"Case #x: "` 뒤에 합을 붙여 출력해야 하며, `x`는 1부터 시작하는 <strong>테스트 케이스 번호</strong>입니다.
...
1부터 100 사이의 숫자 중에서 <strong>7의 배수만 출력하는 프로그램</strong>을 작성하세요.

단, <strong>짝수인 7의 배수는 건너뛰고</strong>, 출력에서 제외하세요.
...
사용자에게 정수를 반복 입력받아 <strong>총합을 누적 계산</strong>하는 프로그램을 작성하세요.

단, <strong>음수를 입력하면 입력을 종료</strong>하고, 마지막에 누적된 총합을 출력하세요.
...
<a id="3-짝수만-제곱하여-출력"></a>

### 3) 짝수만 제곱하여 출력

문제 설명

사용자로부터 10개의 정수를 입력받아, <strong>짝수일 경우에만 제곱을 출력</strong>하는 프로그램을 작성하세요.
...
`2단부터 9단까지`의 <strong>구구단 전체를 출력</strong>하는 프로그램을 작성하세요.
...
사용자로부터 정수 `N(1 ≤ N ≤ 20)`을 입력받아, `N`층의 <strong>왼쪽 정렬된 별 피라미드</strong>를 출력하세요.
