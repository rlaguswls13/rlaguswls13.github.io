...
`devlog-recommendations.json`, `engagement.json`, `config/slugs.json`, `config/routes.json`은 `scripts/`의 npm 작업으로 갱신합니다. 화면용 `indexes/{journal,devlog,projects}.json`은 MDX frontmatter를 기준으로 생성하고, Notion API 원본 맵은 `indexes/notion/`에 분리합니다.
