...
It updates `.wiki/session-memory.md` locally, redacts secret-shaped values, and commits only `project/skills` and `project/hooks`. `.wiki/` is gitignored and never staged or committed to this project's own remote — it never enters this repo's history. `D:\obsidian-storage\project-rag\blog\` is its sole home (that folder itself is the `.wiki` root, git-managed there). It never stages source, generated content, environment files, or unrelated user changes.
