...
커밋·promote 전 변경 범위와 자동 생성 결과를 검증합니다. 원본 checklist는 [`references/pre-commit-checklist.md`](references/pre-commit-checklist.md)이고 pipeline 규칙은 `.wiki/content-pipeline.md`(저장소 밖 `D:\obsidian-storage\project-rag\blog\content-pipeline.md`, `$PROJECT_RAG_PATH` 우선)입니다.
