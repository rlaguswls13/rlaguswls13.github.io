...
네이버 서치어드바이저 사이트 등록과 소유권 verification을 수행합니다. 현재 저장소 구현은 HTML meta tag 방식이며 공개 환경변수 `SEARCH_ADVISER_VERIFICATION`의 content 값만 빌드에 반영합니다. Google Search Console과 동일한 배선 패턴을 사용하므로 [`google-search-console-operations`](../google-search-console-operations/SKILL.md)와 함께 읽습니다.
...
4. GitHub Environment `github-pages`의 공개 Variable `SEARCH_ADVISER_VERIFICATION`에 content 값을 설정합니다. 로컬 검증용으로는 `.env.local.yml`의 `SEARCH_ADVISER_VERIFICATION=`에 채워 넣습니다(커밋 금지).
...
- `소유확인 실패`: 배포된 homepage의 최종 HTML `<head>`에 tag가 있는지, `SEARCH_ADVISER_VERIFICATION` Variable이 실제로 설정되었는지 확인합니다(`generate-build-resources` 실행 시 값이 비어 있으면 meta tag 자체가 생성되지 않습니다).
